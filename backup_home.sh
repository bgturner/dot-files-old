...
tar cvzf $TEMPDIR/home-config-$NOW.tar.gz --exclude-caches-all --exclude-backups --exclude 'chromium/*' --exclude '.config/VirtualBox/*' --exclude '*/.git' --exclude '.local/share/Trash/*' .config .cinnamon .dot-files .filezilla .gnucash .icons .local .mypaint .ssh .themes .wallpaper .xmind .face
